      ...
      readXlsxFile(bufferToStream(req.file.buffer)).then(async (rows) => {
        ...
        try {
          ...
          if (error.response) {
            res
              .status(502)
              .json({ msg: "Không thể tạo tx: " + error.response.data.error });
          } else {
...
async function getBureauById(bureauId) {
  ...
  const doc = await bureauHistoryCol.findOne(
    { "profiles.bureauId": bureauId },
    { projection: { "profiles.$": 1, _id: 0 } }
  );
  return doc ? doc.profiles[0] : null;
}
async function getTeacherById(teacherId) {
  const teacherHistoryCol = (await connection)
    .db()
    .collection("TeacherHistory");
  const doc = await teacherHistoryCol.findOne(
    { "profiles.teacherId": teacherId },
    { projection: { "profiles.$": 1, _id: 0 } }
  );
  return doc ? doc.profiles[0] : null;
}
